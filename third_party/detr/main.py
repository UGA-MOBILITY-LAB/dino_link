...
def get_args_parser():
    ...
    parser.add_argument('--use_dinolink_tokens', action='store_true',
                        help="Use DinoLink decoded tokens as DETR queries (bypass DETR transformer)")
    parser.add_argument('--dinolink_cfg', type=str, default='',
                        help="Path to DinoLink config yaml (required when --use_dinolink_tokens)")
    parser.add_argument('--dinolink_ckpt', type=str, default='',
                        help="Path to DinoLink checkpoint containing projector/quantizer/token_decoder")
    parser.add_argument('--freeze_dinolink', dest='freeze_dinolink', action='store_true',
                        help="Freeze DinoLink modules during DETR training")
    parser.add_argument('--no_freeze_dinolink', dest='freeze_dinolink', action='store_false',
                        help="Allow DinoLink projector/quantizer/decoder to finetune with DETR")
    parser.set_defaults(freeze_dinolink=True)
    # * Backbone
...
def main(args):
    ...
    if args.resume:
        ...
        # Do NOT add prefix when loading our own saved checkpoint (already has detr.* + extractor.* etc).
        if getattr(args, "use_dinolink_tokens", False):
            model_keys = list(model_without_ddp.state_dict().keys())
            ...
            wrapper_prefixes = ("extractor.", "selector.", "projector.", "token_proj.", "token_decoder.", "quantizer.")
        ...
        try:
            ...
            # Allow fine-tuning with different backbone / num_classes by only loading shape-matched tensors.
            ...
            print(f"Skipped checkpoint keys (e.g. class_embed/backbone extras): {len(skipped)}")
