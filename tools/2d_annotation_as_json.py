...
def main(args):
    ...
    out_path = os.path.join(dest_path, args.filename)
    with open(out_path, 'w') as fh:
        json.dump(reprojections, fh, sort_keys=True, indent=4)

    print(f"Saved the 2D re-projections under {out_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Directory to write output JSON (default: dataroot/version; use this if dataroot is read-only).',
    )
